...
exports.knownPrintLabels = exports.getPrintLabel = exports.derivePrintFromProduct = exports.detectPrintVariant = exports.normalizePrint = exports.deriveFinishKey = exports.isFoilVariantKey = exports.isFinishKey = void 0;
const PRINT_LABEL_LOOKUP = {
    base: 'Base',
    foil: 'Foil',
...
const isFinishKey = (key) => (key ? FINISH_KEYS.has(key) : false);
exports.isFinishKey = isFinishKey;
const isFoilVariantKey = (key) => (key ? FOIL_VARIANT_KEYS.has(key) : false);
exports.isFoilVariantKey = isFoilVariantKey;
const deriveFinishKey = (key) => {
    if (!key || key === 'base') {
        return 'base';
    }
    if (key === 'foil' || (0, exports.isFoilVariantKey)(key)) {
        return 'foil';
    }
    return key;
};
exports.deriveFinishKey = deriveFinishKey;
...
exports.normalizePrint = normalizePrint;
const isExtendedDataEntry = (entry) => entry && typeof entry === 'object';
const extractCandidateFromExtendedData = (product) => {
    const extended = Array.isArray(product.extended_data) ? product.extended_data : [];
    for (const entry of extended) {
        if (!isExtendedDataEntry(entry))
            continue;
...
const collectVariantSegments = (value, options = {}) => {
    if (!value) {
        return [];
    }
...
const keywordMatchesSegment = (segment, keyword) => {
...
const resolveVariantFromSources = (sources) => {
 ...
 * })
 */
const extractVariantSources = (input) => {
    if (!input) {
        return [];
    }
    const sources = [];
...
const detectPrintVariant = (input) => {
...
exports.detectPrintVariant = detectPrintVariant;
const derivePrintFromProduct = (product) => {
    if (!product) {
        return (0, exports.normalizePrint)();
    }
    const variant = (0, exports.detectPrintVariant)(product);
    if (variant.key && !(0, exports.isFinishKey)(variant.key)) {
        return variant;
    }
    if (product.finish) {
        const finishCandidate = (0, exports.normalizePrint)(product.finish);
        if ((0, exports.isFoilVariantKey)(finishCandidate.key)) {
            return finishCandidate;
        }
    }
    if (product.print) {
        const normalized = (0, exports.normalizePrint)(product.print);
        if (!(0, exports.isFinishKey)(normalized.key)) {
            return normalized;
        }
    }
    const extendedCandidate = extractCandidateFromExtendedData(product);
    if (extendedCandidate) {
        const normalized = (0, exports.normalizePrint)(extendedCandidate);
        if (!(0, exports.isFinishKey)(normalized.key)) {
            return normalized;
        }
    }
    ...
    if (nameCandidate) {
        const normalized = (0, exports.normalizePrint)(nameCandidate);
        if (!(0, exports.isFinishKey)(normalized.key)) {
            return normalized;
        }
    }
...
exports.derivePrintFromProduct = derivePrintFromProduct;
const getPrintLabel = (key) => ensureLabel(key);
exports.getPrintLabel = getPrintLabel;
const knownPrintLabels = () => ({ ...PRINT_LABEL_LOOKUP });
exports.knownPrintLabels = knownPrintLabels;
//# sourceMappingURL=print-normalizer.js.map